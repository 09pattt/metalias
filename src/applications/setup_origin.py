...
if __name__ == "__main__":
    # config["abspath"] = os.path.dirname(os.path.abspath(__file__))
    # config["requirement"] = os.path.join(config["abspath"], "..", "..", "requirements.txt")
    config["abspath"] = file_utils.appdir()
    config["requirement"] = file_utils.join(file_utils.appdir(), "requirements.txt")

    if "help" in sys.argv or "-h" in sys.argv: # install missing package or correct the version and fix .metalias directory
        print("help, -h : Show setup function manual.")
        print("verbose, -v : Return process statement on console.")
        print("check, -c : - Only check packages and return result without installation.")
        sys.exit(0)
